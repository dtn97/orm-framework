...
namespace LambdaSqlBuilder
{
    ...
    public class SqlLam<T> : SqlLamBase where T : class, new()
    {
        public SqlLam()
        {
            _builder = new SqlQueryBuilder(LambdaResolver.GetTableName<T>());
        ...
        public SqlLam<TResult> Join<T2, TKey, TResult>(SqlLam<T2> joinQuery,
            ...
            Func<T, T2, TResult> selection) where TResult : class, new() where T2 : class, new()
        {
        ...
        public SqlLam<T2> Join<T2>(Expression<Func<T, T2, bool>> expression) where T2 : class, new()
        {
        ...
        public SqlLam<T> HavingCount(Expression<Func<T, object>> expression, Expression<Func<int, bool>> cmp)
        {
            _resolver.HavingWithCount(expression, SelectFunction.COUNT, cmp);
            return this;
        }

        public SqlLam<T> HavingSum(Expression<Func<T, object>> expression, Expression<Func<int, bool>> cmp)
        {
            _resolver.HavingWithCount(expression, SelectFunction.SUM, cmp);
            return this;
        }

        public SqlLam<T> HavingAverages(Expression<Func<T, object>> expression, Expression<Func<int, bool>> cmp)
        {
            _resolver.HavingWithCount(expression, SelectFunction.AVG, cmp);
            return this;
        }
